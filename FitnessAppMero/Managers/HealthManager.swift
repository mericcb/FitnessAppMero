...
extension HealthManager {
    ...
    func fetchYTDAndOneYearData(completion: @escaping (Result<YearChartDataResult, Error>) -> Void) {
        ...
        for i in 0..<11 {
            let month = Calendar.current.date(byAdding: .month, value: -i, to: Date()) ?? Date()
            let (startMonth, endMonth) = month.fetchMonthStartAndEnddate()
            let predicate = HKQuery.predicateForSamples(withStart: startMonth, end: endMonth)
            let query = HKStatisticsQuery(quantityType: steps, quantitySamplePredicate: predicate) { _, results, error in
